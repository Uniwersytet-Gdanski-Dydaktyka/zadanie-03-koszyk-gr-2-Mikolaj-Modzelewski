...
class PromotionsTest {
    ...
    void shouldAddMugWhenTotalOver200() {
        ...
        List<Product> result = promo.apply(items);
        assertEquals(2, result.size());
        assertEquals("mug-brnd", result.get(1).getCode());
        assertEquals(0.0, result.get(1).getDiscountPrice());
    }
    ...
    void shouldApply5PercentDiscountWhenTotalOver300() {
        ...
        List<Product> items = Arrays.asList(new Product("001", "P1", 400));

        List<Product> result = promo.apply(items);
        assertEquals(380.0, result.get(0).getDiscountPrice(), 0.01);
    }

    @Test
    void shouldNotApply5PercentDiscountWhenTotalIs300OrLess() {
        Promotion promo = new MoreThan300();
        List<Product> items = Arrays.asList(new Product("001", "P1", 300));

        List<Product> result = promo.apply(items);
        assertEquals(300.0, result.get(0).getDiscountPrice());
    }
    ...
    void shouldGiveCheapestProductForFreeWhen3OrMoreItems() {
        ...
        List<Product> result = promo.apply(items);
        assertEquals(100.0, result.get(0).getDiscountPrice());
        assertEquals(0.0, result.get(1).getDiscountPrice());
        assertEquals(200.0, result.get(2).getDiscountPrice());
    }
    ...
    void shouldNotApplyTwoPlusOneWhenLessThan3Items() {
        ...
        List<Product> result = promo.apply(items);
        assertEquals(100.0, result.get(0).getDiscountPrice());
        assertEquals(50.0, result.get(1).getDiscountPrice());
    }
    ...
    void shouldApply30PercentDiscountToSpecificProduct() {
        ...
        List<Product> result = promo.apply(items);
        assertEquals(100.0, result.get(0).getDiscountPrice());
        assertEquals(70.0, result.get(1).getDiscountPrice(), 0.01);
    }
}